...
function csv_to_json(filePath){
    ...
    // hack result to look like the data
    json_object = {
        data: {
            item: result
        }
    ...
    let jsonResult = JSON.stringify({data: {item: result}});
    let jsonFile = 'output.json'
    fs.writeFileSync(jsonFile, jsonResult);
    readAndProcessJson(jsonFile);
...
function readAndProcessJson(file) {
    fs.readFile(path.join(extractPath, file), function (err, data) {
        if (err) {
            logger.error("Error reading file:", err.message, file);
            return;
        }
...
function unzip_data() {
    downloadFile().catch((err) => {
        logger.error("Error during download of csv:", err.message);
    });
    csv_to_json(filePath);
}
unzip_data();
job.start();
